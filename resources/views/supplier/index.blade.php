...
@section('content')
                    ...
                    @foreach ($suppliers as $index => $supplier)
                        <tr class="transition-all hover:bg-[#f9f9f9]">
                            ...
                            <td class="px-6 py-4 flex gap-2">
                                <button data-modal-target="edit-paket-modal-{{ $supplier->id }}"
                                    data-modal-toggle="edit-paket-modal-{{ $supplier->id }}"
    ...
    @foreach ($suppliers as $supplier)
        ...
        <div id="edit-paket-modal-{{ $supplier->id }}" tabindex="-1" aria-hidden="true"
            class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative w-full max-w-lg max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                        ...
                        <button type="button"
                            class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white"
                            data-modal-hide="edit-paket-modal-{{ $supplier->id }}">
                        ...
                        <form action="{{ route('admin.supplier.update') }}" method="POST">
                            ...
                            <div class="flex gap-2 w-full">
                                <div class="mb-4 w-full">
                                    ...
                                    <input type="text" id="alamat" name="alamat" value="{{ $supplier->alamat }}"
                                        class="block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-sm focus:border-primary focus:outline-primary focus:ring-primary dark:text-white">
                                    @error('alamat')
                                        <div class="text-red-500">{{ $message }}</div>
                                    @enderror
