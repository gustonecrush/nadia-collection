...
@section('content')
    <div class="flex flex-col gap-10">
        <div class="flex justify-between items-center mb-20">
            <div class="flex flex-col ">
                <h1 class="text-2xl font-semibold">🕋 Manajemen Data Hotel Mekah</h1>
                <p class="text-gray-400 text-sm">
                    Lakukan manajemen data hotel mekah untuk dapat dimasukkan pada paket umrah yang tersedia
                </p>
            </div>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($hotelMekahs as $index => $hotelMekah)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <th scope="row"
                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ $index + 1 }}
                            </th>
                            <td class="px-6 py-4">{{ $hotelMekah->nama_hotel_mekah }}</td>
                            <td class="px-6 py-4 flex gap-2">
                                <button data-modal-target="edit-hotel-modal" data-modal-toggle="edit-hotel-modal"
                                    data-hotel-id="{{ $hotelMekah->id }}"
                                    ...
                                    <div class="relative p-4 w-full max-w-2xl max-h-full">
                                        ...
                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                            ...
                                            <div
                                                ...
                                                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Edit Hotel
                                                    Mekah</h3>
                                                <button type="button"
                                            ...
                                            <div class="p-4 md:p-5 space-y-4">
                                                <form action="{{ route('admin.hotel-mekah.update') }}" method="POST">
                                                    @csrf
                                                    @method('POST')
                                                    <input type="hidden" id="edit_hotel_id" name="id">
                                                    <input type="hidden" name="id" value="{{ $hotelMekah->id }}">
                                                    <div class="mb-4">
                                                        <div class="mb-2">
                                                            <label for="edit_nama_hotel_mekah"
                                                                class="block mb-2 font-medium text-gray-500 dark:text-white text-sm">Nama
                                                                Hotel</label>
                                                            <input type="text"
                                                                placeholder="{{ $hotelMekah->nama_hotel_mekah }}"
                                                                id="edit_nama_hotel_mekah" name="nama_hotel_mekah"
                                ...
                                <form action="{{ route('admin.hotel-mekah.delete', $hotelMekah) }}" method="POST"
                                    onsubmit="return confirm('Are you sure?');">
                                    @csrf
                                    @method('DELETE')
                                    <input type="hidden" value="{{ $hotelMekah->id }}" name="id">
                                    <button type="submit"
        ...
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                    <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Tambah Hotel Mekah</h3>
                    <button type="button"
                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                ...
                <!-- Modal body -->
                <div class="p-4 md:p-5 space-y-4">
                    <form action="{{ route('admin.hotel-mekah.store') }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <div class="mb-2">
                                <label for="nama_hotel_mekah"
                                    class="block mb-2 font-medium text-gray-500 dark:text-white text-sm">Nama Hotel</label>
                                <input type="text" id="nama_hotel_mekah" name="nama_hotel_mekah"
                                    class="block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-sm  focus:border-[#543310] focus:outline-[#543310] focus:ring-[#543310]  dark:text-white ">
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const editNamaHotelMekah = document.getElementById('edit_nama_hotel_mekah');
            const editHotelId = document.getElementById('edit_hotel_id');

            editButtons.forEach(button => {
                button.addEventListener('click', async function() {
                    const hotelId = this.getAttribute('data-hotel-id');
                    const response = await fetch(`/hotel-mekah/${hotelId}`);
                    const hotel = await response.json();

                    editNamaHotelMekah.value = hotel.nama_hotel_mekah;
